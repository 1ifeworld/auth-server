...
app.post('/provisionSession', async (c) => {
    try {
      ...
      if (!siweMsg && !sessionId) {
        return c.json({ success: false, message: 'Missing SIWE message or session ID' }, 400)
      }
      ...
      if (!deviceId) {
        if (!siweMsg) {
          return c.json({ success: false, message: 'Missing SIWE message for new user' }, 400)
        }
        ...
        newDeviceId = generateRandomString(10, alphabet('a-z', 'A-Z', '0-9', '-', '_'))
        userId = 25 
        ...
        const insertKeysQuery = `
          ...
          VALUES ($1, $2, $3)
        `
        await writeClient.query(insertKeysQuery, [userId, custodyAddress, newDeviceId])
      } 
